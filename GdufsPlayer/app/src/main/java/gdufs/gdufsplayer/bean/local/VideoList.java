...
public class VideoList {
    ...
    public boolean removeImage(VideoObject image) {
        ...
        if (isIdExist(image.mId)) {
            if (LocalVideoUtil.isFileExist(image.getMediapath())) {
    ...
    public boolean renameImage(Context context, VideoObject image, String name) {
        ...
        String newPath = LocalVideoUtil.replaceFilename(oldpath, name);
        ...
        if (name != null && name.length() > 0) {
            ...
            mContentResolver.update(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, values,
                    MediaStore.Video.Media._ID + "=?", new String[] {
...
//                    + newPath)));
    ...
    public boolean isFilenameExist(VideoObject image, String name) {
        String newPath = LocalVideoUtil.replaceFilename(image.getMediapath(), name);
        boolean exist = false;
        ...
        synchronized (c) {
            try {
                ...
                while (!c.isAfterLast()) {
                    ...
                    if (LocalVideoUtil.getBucketId(newPath) == LocalVideoUtil.getBucketId(oldpath)) {
                        exist = true;
                        break;
                    }
                    c.moveToNext();
